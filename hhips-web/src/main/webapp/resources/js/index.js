	...
	*/
	...
	$.post('/'+contextPath+'/menu',null,function(data){
		$.each(data,function(index,item){
			menuHtml += "<li class='sub-menu sytem_menus'><a class='waves-effect'><i class='"+item.icon+"'></i> "+item.name+"</a><ul>";
			$.each(item.children,function(ids,itm){
				menuHtml += "<li><a class='waves-effect' href='javascript:Tab.addTab(\""+itm.name+"\", \"/"+contextPath+itm.url+"\");'>"+itm.name+"</a></li>";
			});
			menuHtml += "</ul></li>";
		});
